...
_COLLECT_TEMP_TABLES = frozenset({"retl_state_collect_snapshot", "retl_event_collect_window"})


class DatabricksSqlDialect(SimpleSqlDialect):
    """Databricks SQLGlot rendering and backend-owned SQL capability helpers."""
    ...
    def executable_collect_runtime_store_label(self) -> str:
        return "DatabricksSqlBackend-owned runtime store"
    ...
    def source_relation(
        ...
        self._validate_relation_space(
            ...
            context="Databricks Source relation space",
        )
        return RelationPath(
            name=relation,
    ...
    def runtime_relation(
        ...
        self._validate_relation_space(
            ...
            context="Databricks Runtime relation space",
        )
        return RelationPath(
            name=relation,
    ...
    def json_array_sql(self, values: list[str]) -> str:
        return f"array({', '.join(values)})" if values else "array()"
    ...
    def identifier_list_array_sql(self, *, identifier_type: str, values_sql: str) -> str:
        value_sql = "cast(retl_identifier_value as string)"
        return (
    ...
    def json_extract_scalar_sql(self, json_sql: str, path: str) -> str:
        return f"get_json_object({json_sql}, {self.sql_literal(path)})"
    ...
    def temp_relation(self, name: RelationName | str) -> RelationPath:
        self._validate_collect_temp_name(str(name))
    ...
    def drop_temp_table_sql(self, name: str) -> str:
        return f"drop temporary table if exists {self.render_temp_relation(name)}"
    ...
    def update_many_sql(
        ...
        *,
        ...
        match_sql = " AND ".join(
            f"target.{self.quote_identifier(column_name.value)} = "
            f"source.{self.quote_identifier(column_name.value)}"
            for column_name in key_columns
        )
        assignments = [
            f"{self.quote_identifier(column_name.value)} = "
            f"source.{self.quote_identifier(column_name.value)}"
    ...
    def _validate_relation_space(
        ...
        *,
        ...
        if space.backend_name != "databricks":
            raise ValueError(f"{context} backend must be databricks.")
        ...
        if space.database.casefold() == "hive_metastore":
            raise ValueError("Databricks relation spaces do not support hive_metastore.")

    def _validate_collect_temp_name(self, name: str) -> None:
        if name not in _COLLECT_TEMP_TABLES:
            raise ValueError(
                "Databricks temporary tables are only supported for RETL collect scratch tables."
            )


DATABRICKS_DIALECT = DatabricksSqlDialect()


__all__ = ["DATABRICKS_DIALECT", "DatabricksSqlDialect"]
